...
interface PenilaianProps {
  onSave: (data: NewSyahadahEntry, id?: string) => Promise<void>;
  syahadahToEdit?: SyahadahData | null;
  onCancelEdit: () => void;
}
...
const Penilaian: React.FC<PenilaianProps> = ({ onSave, syahadahToEdit, onCancelEdit }) => {
    ...
    useEffect(() => {
        if (syahadahToEdit) {
            setFormData({
                namaSiswa: syahadahToEdit.namaSiswa,
                kelas: syahadahToEdit.kelas,
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 text-center">
                        ...
                        <div className="mt-4 space-y-2">
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-300 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center"
                            >
                                {isSaving ? (
                                    <>
                                    ...
                                    syahadahToEdit ? 'Update Penilaian' : 'Simpan Penilaian'
                                )}
                            </button>
                             {syahadahToEdit && (
                                <button
                                    onClick={onCancelEdit}
                                    className="w-full bg-gray-200 text-gray-800 font-bold py-3 px-4 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 transition-colors duration-300"
                                >
                                    Batal
                                </button>
                            )}
